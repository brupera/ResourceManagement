...
class TimeLineCalendar():
    ...
    def get_date_range_header(self, start_date, end_date):
        ...
        while (start_date <= (end_date)):
            date_td += '<th scope="row" style="text-align: center;vertical-align: middle;">{}<br>{}</th>'.format(
                start_date.strftime("%d"), start_date.strftime("%a")[0])
            start_date += delta
        ...
        while (start_date <= end_date):
            ...
            for allocation in allocations:
                if start_date.weekday() == allocation.day_of_week and \
                    ...
                    allocation_data = '<td bgcolor="{}" class="allocation-td {}" onclick="allocation_edit({})" ></td>'.format(
                        allocation.color_code, class_name,
            ...
            for allocation in allocations:
                if start_date.weekday() == allocation.day_of_week and \
                    ...
                    allocation_data = '<td bgcolor="{}">' \
                                      '<a href="/allocation/update/{}">{}</a></td>'.format(allocation.color_code,
                                                                                           allocation.allocation_id,
                                                                                           allocation.project)
                    total_allocation_hours += allocation.no_of_hours
                    break
        ...
        for employee in Employee.objects.all():
            ...
            for project in allocated_projects:
                ...
                if not employe_info_tds_created:
                    ...
                    timeline_tr += '<th class="text-nowrap" style="vertical-align: middle;" rowspan="{}">{}</th>'.format(
                        row_span, employee.job_title.name)
